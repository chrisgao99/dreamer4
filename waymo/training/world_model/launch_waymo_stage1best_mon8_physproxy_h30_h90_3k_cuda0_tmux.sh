#!/usr/bin/env bash
# Stage-1 best -> H30 3k -> H90 3k with joint MoN N=8 and decoded
# fixed-sedan collision / all-road-edge offroad / kinematic proxy losses.
...
H30_RUN="waymo_wm_stage1best_mon8_physproxy_ctx1_h30_d1_chunk32s30_3k"
H90_RUN="waymo_wm_stage1best_mon8_physproxy_h30step3k_ctx1_h90_d1_chunk32s30_3k"
...
SESSION_NAME="${SESSION_NAME:-wm_stage1best_mon8_physproxy_h30_h90_3k_cuda0}"
...
{
  echo "===== $(date) MoN8 physical-proxy H30->H90 pipeline start ====="
  ...
  echo "collision=fixed_4.8x2.0m projected_clearance warning=1.0m weight=0.1"
  echo "offroad=fixed_4.8x2.0m corners all_road_edges_15_16 margin=0.3m weight=0.1"
  echo "kinematic_weights=vxy:5,speed_yaw:2 shortcut_retention_weight=0.1 warmup=500"
} | tee -a "$PIPELINE_LOG"
...
common_args=(
  ...
  --log_every 10 --eval_every 1000 --eval_max_batches 8 --save_every 500 --no-save_latest_each_epoch
...
echo "===== $(date) MoN8 physical-proxy pipeline complete =====" | tee -a "$PIPELINE_LOG"
