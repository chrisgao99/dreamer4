#!/usr/bin/env bash
# Stage-1 best -> H30 3k -> H90 3k. One free-running rollout is trained only
# with full-future raw motion-state supervision through the frozen decoder.
...
H30_RUN="waymo_wm_stage1best_n1_fullmotion_only_ctx1_h30_d1_chunk32s30_3k"
H90_RUN="waymo_wm_stage1best_n1_fullmotion_only_h30step3k_ctx1_h90_d1_chunk32s30_3k"
...
SESSION_NAME="${SESSION_NAME:-wm_stage1best_n1_fullmotion_only_h30_h90_3k_cuda2}"
...
{
  echo "===== $(date) N1 full-motion-only H30->H90 pipeline start ====="
  ...
  echo "objective=rollout_motion N=1 ctx=1 sampler=D1 batch=1"
  echo "optimization_loss=full_future_motion_gt_only xy:1 velocity:0.5 yaw:0.5 weight=1"
  echo "excluded=latent_mse mon collision offroad kinematic_self_consistency shortcut_retention"
...
common_args=(
  ...
  --agent_kinematic_xy_weight 0 --agent_speed_yaw_kinematic_weight 0 --kinematic_dt 0.1
...
echo "===== $(date) N1 full-motion-only pipeline complete =====" | tee -a "$PIPELINE_LOG"
