...
TOKENIZER_DIR="${TOKENIZER_DIR:-$REPO_ROOT/waymo/checkpoints/ooi50k_lat64_b32_d256_ep200_anygpu_staticmap_v2_chunk32_raw_kinematic_nofde_focus_randstart_decmap_noamp}"
TOKENIZER_CKPT="${TOKENIZER_CKPT:-$TOKENIZER_DIR/best.pt}"
DATA_ROOT="${DATA_ROOT:-$REPO_ROOT/waymo/data/waymo_vector_dataset_ooi_centered_training_all}"

RUN_NAME="${RUN_NAME:-wm_ooi_all_tok_lat64_decmap_shortcut_seq32_randstart_d512_l8_b8_bf16_gpu2}"
SESSION_NAME="${SESSION_NAME:-wm_ooi_lat64_gpu2}"
...
if ! compgen -G "$DATA_ROOT/train/*.npz" >/dev/null; then
  echo "No training NPZ files found under: $DATA_ROOT/train" >&2
  exit 1
fi
if ! compgen -G "$DATA_ROOT/val/*.npz" >/dev/null; then
  echo "No validation NPZ files found under: $DATA_ROOT/val" >&2
  exit 1
fi
...
cd "$REPO_ROOT"
export CUDA_VISIBLE_DEVICES OMP_NUM_THREADS WANDB_MODE
...

{
  ...
  echo "tokenizer_mode=frozen; decoder_static_map_conditioning=loaded_from_checkpoint"
