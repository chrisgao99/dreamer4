#!/usr/bin/env bash
# Lightweight focus world model: one agent latent, FiLM shortcut conditioning,
# two registers, and causal temporal attention in every Transformer layer.

set -euo pipefail

REPO_ROOT="${REPO_ROOT:-/p/yufeng/tri30/dreamer4}"
PYTHON="${PYTHON:-/p/yufeng/.conda/envs/dreamer4/bin/python}"
TOKENIZER_CKPT="${TOKENIZER_CKPT:-$REPO_ROOT/waymo/checkpoints/focus_tokenizer_a_agent_token_raw_map/best.pt}"
DATA_ROOT="${DATA_ROOT:-$REPO_ROOT/waymo/data/waymo_vector_dataset_ooi_centered_training_all}"

RUN_NAME="${RUN_NAME:-wm_ooi_all_focus_agenttok256_film_r2_timeall_shortcut_seq32_d512_l8_b8_bf16_gpu0}"
SESSION_NAME="${SESSION_NAME:-wm_focus_agenttok_film_gpu0}"
...

{
  ...
  echo "tokenizer_mode=frozen_focus_agent_token; n_latents=1; d_bottleneck=256"
  echo "dynamics_variant=focus_film; tokens_per_step=3; n_register=2; temporal_attention=every_layer"
  echo "shortcut_conditioning=FiLM(step_embedding+signal_embedding); action_token=none"
  ...
  echo "d_model_dyn=$D_MODEL_DYN dyn_depth=$DYN_DEPTH n_heads=$N_HEADS packing_factor=$PACKING_FACTOR"
