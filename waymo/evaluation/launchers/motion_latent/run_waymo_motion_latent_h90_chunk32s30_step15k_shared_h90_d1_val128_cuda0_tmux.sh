#!/usr/bin/env bash
# Evaluate the current chunk32/stride30 MotionLatent H90 step-15k checkpoint
# on the recorded 128-sample validation subset. All horizon metrics are
# prefixes of one shared ctx=1 -> H90 direct-D1 rollout per sample.
...
EVAL_CKPT="$REPO_ROOT/waymo/checkpoints/waymo_motion_latent_v1_h30final_exact_h90_chunk32s30_b1_30k/step_00015000.pt"
...
RUN_GROUP="motion_latent_h90_chunk32s30_step15k_shared_h90_d1_val128_seed0"
...
SESSION_NAME="${SESSION_NAME:-wm_motion_latent_h90_chunk32s30_step15k_val128_cuda0}"
...
{
  echo "===== $(date) MotionLatent H90 chunk32s30 step15k validation start ====="
  ...
  echo "===== $(date) MotionLatent H90 chunk32s30 step15k validation complete ====="
} 2>&1 | tee -a "$LOG_FILE"
