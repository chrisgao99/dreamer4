#!/usr/bin/env bash
# Evaluate the proxy-best Stage 1/2 SingleQ4 checkpoints for both variants.
# Each variant runs its two checkpoints sequentially on one physical GPU.
...
RUN_GROUP="${RUN_GROUP:-motion_latent_singleq4_best_first_two_stages_ctx1_h80_val128_seed0}"
...
SESSION_NAME="${SESSION_NAME:-wm_singleq4_eval_${VARIANT}_cuda${CUDA_DEVICE}}"
case "$VARIANT" in
  noqgt)
...
for path in "$PYTHON" "$EVAL_SCRIPT" "$TOKENIZER_CKPT" "$SEMANTIC_READER_CKPT" "$STAGE1_CKPT" "$STAGE2_CKPT" "$SUBSET_MANIFEST"; do
...
{
  echo "===== $(date) SingleQ4 validation start ====="
  ...
  echo "semantics=one_q_integration_plus_four_latent_shortcuts_per_predicted_frame"
  echo "metric_horizons=$HORIZONS subset_manifest=$SUBSET_MANIFEST"

  labels=(stage1_best600k stage2_best15k)
  checkpoints=("$STAGE1_CKPT" "$STAGE2_CKPT")
  for index in "${!labels[@]}"; do
    label="${labels[$index]}"
    checkpoint="${checkpoints[$index]}"
    output_json="$OUT_DIR/${label}_ctx1_shared_h80_h10_20_30_50_80_val128.json"
  ...
  echo "===== $(date) SingleQ4 validation complete: $VARIANT ====="
} 2>&1 | tee -a "$LOG_FILE"
