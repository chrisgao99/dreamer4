#!/usr/bin/env bash
# Evaluate Stage-3 H90 step-15k SingleQ4 checkpoints with one shared H90 rollout.
...
SESSION_TAG="${SESSION_TAG:-_stage3_h90_step15k_gtvalidq}"
...
if [[ "${RUN_INSIDE_TMUX:-0}" != "1" ]]; then
  ...
  echo "Logs: $LOG_DIR/${RUN_GROUP}_stage3_15k_{noqgt,qgt_detach}.log"
  exit 0
fi

VARIANT="${VARIANT:?VARIANT is required inside tmux}"
CUDA_DEVICE="${CUDA_DEVICE:?CUDA_DEVICE is required inside tmux}"
SESSION_NAME="${SESSION_NAME:-wm_singleq4_eval_${VARIANT}_stage3_h90_step15k_cuda${CUDA_DEVICE}}"
case "$VARIANT" in
  noqgt)
    EVAL_CKPT="$REPO_ROOT/waymo/checkpoints/waymo_motion_latent_singleq4_noqgt_chunk32s30_stage3_h90_b1_30000/step_00015000.pt"
    ;;
  qgt_detach)
    EVAL_CKPT="$REPO_ROOT/waymo/checkpoints/waymo_motion_latent_singleq4_qgt_detach_chunk32s30_stage3_h90_b1_30000/step_00015000.pt"
    ;;
  *)
...
LOG_FILE="$LOG_DIR/${RUN_GROUP}_stage3_15k_${VARIANT}.log"
OUTPUT_JSON="$OUT_DIR/stage3_h90_15k_ctx1_shared_h90_h10_20_30_50_80_90_val128.json"
...
{
  echo "===== $(date) SingleQ4 Stage-3 H90 step-15k validation start ====="
  ...
  echo "===== $(date) SingleQ4 Stage-3 H90 step-15k validation complete: $VARIANT ====="
} 2>&1 | tee -a "$LOG_FILE"
