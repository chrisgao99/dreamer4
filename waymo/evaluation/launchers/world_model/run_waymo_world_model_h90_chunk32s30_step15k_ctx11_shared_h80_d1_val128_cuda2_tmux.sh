#!/usr/bin/env bash
# Evaluate the H90 chunk32/stride30 World Model step-15k checkpoint with
# 11 GT context frames and one shared H80 D1 rollout on the recorded val128.
...
RUN_GROUP="world_model_h90_chunk32s30_step15k_ctx11_shared_h80_d1_val128_seed0"
...
OUTPUT_JSON="$OUT_DIR/step15k_ctx11_chunk32s30_shared_h80_d1_h10_30_50_80_val128.json"

SESSION_NAME="${SESSION_NAME:-wm_h90_chunk32s30_step15k_ctx11_h80_d1_val128_cuda2}"
...
{
  echo "===== $(date) World Model step15k ctx11 shared-H80 evaluation start ====="
  echo "session=$SESSION_NAME physical_cuda=$CUDA_VISIBLE_DEVICES"
  echo "checkpoint=$EVAL_CKPT"
  echo "protocol=same_recorded_val_samples=128 batch_size=1 ctx=11 shared_rollout_horizon=80 shortcut_d1=1"
  ...
  "$PYTHON" "$EVAL_SCRIPT" \
    ...
    --eval_seq_len 91 --eval_ctx 11 --horizons "$HORIZONS" \
  ...
  echo "===== $(date) World Model step15k ctx11 shared-H80 evaluation complete ====="
} 2>&1 | tee -a "$LOG_FILE"
