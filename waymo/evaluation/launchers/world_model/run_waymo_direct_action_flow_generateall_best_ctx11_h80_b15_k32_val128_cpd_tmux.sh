#!/usr/bin/env bash
# Generate-all best.pt: H15 plans, execute B=15 per replan, H80/K32/512 scenes.
...
COMMITMENT_STEPS=15
RUN_NAME="${RUN_NAME:-waymo_direct_action_flow_generateall_best_ctx11_h80_b15_k32_val128_cpd}"
SESSION_NAME="${SESSION_NAME:-wm_daf_generateall_best_h80_b15_k32_cpd}"
CUDA_DEVICE="${CUDA_DEVICE:-0}"
VAL_DATA="${VAL_DATA:-$REPO_ROOT/data/waymo_vector_dataset_ooi_centered_50k/val}"
EVAL_CKPT="${EVAL_CKPT:-$REPO_ROOT/waymo/checkpoints/waymo_direct_action_flow_v1_h15_b5_generateall_scratch100k_phys5_yaw075_huber_bounded_tmax090_lr5e5/best.pt}"
...
# Fail early when only launchers, but not the updated evaluator, were copied.
if ! "$PYTHON" "$EVAL_SCRIPT" --help | grep -q -- '--focus_mode'; then
  echo "Use the dreamer4 Python and sync the updated eval_waymo_direct_action_flow_multisample.py." >&2
  exit 1
fi
for output in "$OUT_DIR/result.json" "$OUT_DIR/rollout_ade.csv" \
              "$OUT_DIR/scene_metrics.csv" "$OUT_DIR/details.npz" "$LOG_FILE"; do
...
command=("$PYTHON" "$EVAL_SCRIPT"
  ...
  --scene_metrics_csv "$OUT_DIR/scene_metrics.csv" --details_npz "$OUT_DIR/details.npz"
  --device cuda --weights ema --focus_mode generate_all
...
if [[ "${RUN_INSIDE_TMUX:-0}" != 1 ]]; then
  ...
  printf -v tmux_command '%q ' env RUN_INSIDE_TMUX=1 \
    ...
    VAL_DATA="$VAL_DATA" EVAL_CKPT="$EVAL_CKPT" RUN_NAME="$RUN_NAME" \
...
{
  echo "===== $(date -Is) Generate-all ADE/minADE/CPD evaluation start ====="
  echo "host=$(hostname) session=$SESSION_NAME physical_cuda=$CUDA_DEVICE"
  echo "checkpoint=$EVAL_CKPT weights=ema focus_future=not_conditioned generated=all_agents"
  echo "protocol=context11 native_horizon15 commitment${COMMITMENT_STEPS} rollout80 solver_steps8 seed12346"
  echo "evaluation=batches128 batch_size4 rollouts32 scenes512; ADE/CPD include focus"
  "${command[@]}"
  echo "===== $(date -Is) Generate-all ADE/minADE/CPD evaluation complete ====="
} 2>&1 | tee "$LOG_FILE"
