#!/usr/bin/env bash
# Oracle corrective-action evaluation for the final map-conditioned world model on CUDA 0.

set -euo pipefail

REPO_ROOT="${REPO_ROOT:-/p/yufeng/tri30/dreamer4}"
PYTHON="${PYTHON:-/p/yufeng/.conda/envs/dreamer4/bin/python}"
EVAL_SCRIPT="$REPO_ROOT/waymo/evaluation/eval_waymo_world_model_oracle_corrective_horizons.py"
...
SESSION_NAME="${SESSION_NAME:-wm_mapx1_oracle_corrective_ctx1_horizons_cuda0}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
HORIZONS="${HORIZONS:-10 30 50 80 90}"
EVAL_BATCH_SIZE="${EVAL_BATCH_SIZE:-4}"
EVAL_MAX_BATCHES="${EVAL_MAX_BATCHES:-128}"
NUM_WORKERS="${NUM_WORKERS:-4}"
ORACLE_DECODE_WINDOW="${ORACLE_DECODE_WINDOW:-0}"
PROGRESS_EVERY="${PROGRESS_EVERY:-4}"

OUT_DIR="${OUT_DIR:-$REPO_ROOT/waymo/evaluation/reports/$RUN_NAME}"
OUTPUT_JSON="${OUTPUT_JSON:-$OUT_DIR/final_ctx01_horizons_10_30_50_80_90_batches${EVAL_MAX_BATCHES}_oracle_corrective.json}"
LOG_DIR="${LOG_DIR:-$REPO_ROOT/waymo/logs/evaluation}"
LOG="${LOG:-$LOG_DIR/${RUN_NAME}_final_ctx01_horizons_cuda0_oracle_corrective.log}"
...
if [[ "${RUN_INSIDE_TMUX:-0}" != "1" ]]; then
  ...
  launch_cmd=(
    ...
    "ORACLE_DECODE_WINDOW=$ORACLE_DECODE_WINDOW"
...

{
  echo
  echo "===== $(date) ====="
  echo "mode=oracle_corrective_focus"
  ...
  echo "eval_seq_len=91 max_rollout_window=11 oracle_decode_window=$ORACLE_DECODE_WINDOW"
