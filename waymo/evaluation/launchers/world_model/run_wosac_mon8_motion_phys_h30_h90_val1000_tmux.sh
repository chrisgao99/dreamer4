#!/usr/bin/env bash
# Run one MoN8 full-motion + physical-proxy checkpoint on the same fixed
# 1,000-view local WOSAC protocol used by the stage-3 40k reference run.
...
case "$STAGE" in
  h30)
    LABEL="mon8_motion_phys_h30_step21000"
    WORLD_MODEL_CKPT="$BASE/waymo_wm_stage1best_mon8_fullmotion_physproxy_ctx1_h30_d1_chunk32s30_30k_from3k/step_00021000.pt"
    ;;
  h90)
    LABEL="mon8_motion_phys_h90_step27000"
    WORLD_MODEL_CKPT="$BASE/waymo_wm_stage1best_mon8_fullmotion_physproxy_h30best30k_ctx1_h90_d1_chunk32s30_30k/step_00027000.pt"
    ;;
  *)
...
NUM_VIEWS=1000
TOTAL_VIEWS=1000
# WOSAC requires exactly 32 joint rollouts. MoN8 describes model training,
# while n_register=8 below describes the checkpoint architecture.
NUM_ROLLOUTS=32
...
SESSION_NAME="${SESSION_NAME:-wosac_${LABEL}_val1000_cuda${CUDA_DEVICE}}"
RUN_ID="${RUN_ID:-wosac_${LABEL}_val1000_k4_rb${ROLLOUT_BATCH_SIZE}}"
...
if [[ "${RUN_INSIDE_TMUX:-0}" != "1" ]]; then
  ...
  printf -v tmux_command '%q ' env \
    ...
    REPO_ROOT="$REPO_ROOT" PYTHON="$PYTHON" WOSAC_PYTHON="$WOSAC_PYTHON" \
...

{
  ...
  echo "===== $(date) local WOSAC $LABEL val1000 start ====="
  ...
  "$WOSAC_PYTHON" -c 'import tensorflow as tf; g=tf.config.list_physical_devices("GPU"); assert g, "TensorFlow cannot see the selected CUDA device"; print("tensorflow_gpus="+str(g))'
  ...
  echo "model=MoN8_fullmotion_physical_proxy architecture_n_register=8"
  ...
  echo "===== $(date) local WOSAC $LABEL val1000 complete ====="
