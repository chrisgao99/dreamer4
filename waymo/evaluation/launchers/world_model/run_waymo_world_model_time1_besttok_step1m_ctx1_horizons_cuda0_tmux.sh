#!/usr/bin/env bash
# Evaluate the 1M-step time-every-layer world model from one context frame on CUDA 0.
...
RUN_NAME="waymo_wm_v1_egoact_focus_raw_noclamp_win11_ctx1_h10_randstart_b2_norecon_time1_besttok_1m"
EVAL_CKPT="${EVAL_CKPT:-$REPO_ROOT/waymo/checkpoints/$RUN_NAME/step_01000000.pt}"
...
SESSION_NAME="${SESSION_NAME:-wm_time1_besttok_step1m_ctx1_horizons_cuda0}"
...
OUTPUT_JSON="${OUTPUT_JSON:-$OUT_DIR/step_01000000_ctx01_horizons_10_30_50_80_90_batches${EVAL_MAX_BATCHES}.json}"
LOG_DIR="${LOG_DIR:-$REPO_ROOT/waymo/logs/evaluation}"
LOG="${LOG:-$LOG_DIR/${RUN_NAME}_step1m_ctx01_horizons_cuda0.log}"
...

{
  ...
  echo "eval_seq_len=91 max_rollout_window=11 time_every=1"
