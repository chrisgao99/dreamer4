...
OUT_DIR="${OUT_DIR:-waymo/evaluation/reports/reconstruction_fulltraj_random10_staticmap_v2_chunk${TIME_WINDOW}}"
CKPT="waymo/checkpoints/ooi50k_lat16_d256_ep200_2a100_staticmap_v2_lossfix/latest.pt"
...
while IFS= read -r npz; do
  ...
  python waymo/evaluation/visualize_vector_tokenizer_reconstruction.py \
    --npz "$npz" \
    --checkpoint "$CKPT" \
    --label "staticmap_v2_chunk${TIME_WINDOW}" \
