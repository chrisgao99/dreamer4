#!/usr/bin/env bash
# Render eight stochastic context-1, future-90 rollouts for the previously
# used Waymo scenario, using one selected N=8 + Motion GT checkpoint.
...
SESSION_NAME="${SESSION_NAME:-wm_vis_motion_gt_${STAGE}_scenario7704_cuda${CUDA_DEVICE}}"
PIPELINE_LOG="$LOG_DIR/${SESSION_NAME}.log"
mkdir -p "$OUT" "$LOG_DIR" /tmp/matplotlib-waymo-world-model
...
export MPLCONFIGDIR=/tmp/matplotlib-waymo-world-model

{
  echo "===== $(date) $MODEL_LABEL visualization start ====="
  "$PYTHON" "$SCRIPT" \
    --data_dir "$SCENARIO" --tokenizer_ckpt "$TOKENIZER" --device cuda \
    --eval_ckpt "$CKPT" --scenario_npz "$SCENARIO" --output_dir "$OUT" \
    --model_label "$MODEL_LABEL" \
    --eval_seq_len 91 --eval_ctx 1 --eval_horizon 90 \
  ...
  echo "===== $(date) $MODEL_LABEL visualization complete ====="
